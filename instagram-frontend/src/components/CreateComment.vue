  ...
  <div class="Post__create">
    <section class="Post__create-comment">
      <div class="Post__create-comment-input">
        <textarea aria-label="Add a comment..." ref="input" placeholder="Add a comment..." @keydown.enter.prevent="saveComment" v-model="text" class="Post__create-comment--field"></textarea>
      </div>
    </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveComment() {
      console.log('comment text: ', this.text);
      console.log('comment postId: ', this.postId);
      console.log('comment userId: ', this.userId);
